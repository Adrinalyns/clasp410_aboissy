#!/usr/bin/env python3
'''
This code is example of taylor exapnsion
'''

import numpy as np
import matplotlib.pyplot as plt

dx=0.2
x=np.arange(0,6*np.pi,dx)

sinx=np.sin(x)
cosx=np.cos(x)

fwd_diff=(sinx[1:]-sinx[:-1])/dx
bkd_diff=(sinx[1:]-sinx[:-1])/dx
cnt_diff=(sinx[2:]-sinx[:-2])/(2*dx)

plt.plot(x,cosx, label=r'Analytical Derivative of $\sin{x}$')
plt.plot(x[:-1],fwd_diff, label=r'Forward diff approximation ')
plt.plot(x[1:],bkd_diff, label=r'Backward diff approximation ')
plt.plot(x[1:-1],cnt_diff, label=r'Central diff approximation ')
plt.legend()

#Plotting the error between numerical and analytical

dxs=np.array([2**-n for n in range(20)])
err_fwd,err_cnt=[],[]

for dx in dxs:
    x=np.arange(0,2.5*np.pi,dx)

    sinx=np.sin(x)
    cosx=np.cos(x)

    fwd_diff=(sinx[1:]-sinx[:-1])/dx
    cnt_diff=(sinx[2:]-sinx[:-2])/(2*dx)

    err_fwd.append(np.abs(fwd_diff[-1]-np.cos(x[-1])))
    err_cnt.append(np.abs(cnt_diff[-1]-np.cos(x[-2])))

fig,ax=plt.subplots(1,1)
ax.loglog(dxs,err_fwd,'.', label='Forward diff')
ax.loglog(dxs,err_cnt,'.', label='Central diff')
ax.set_xlabel("dx")
ax.set_ylabel("error")

plt.legend()


